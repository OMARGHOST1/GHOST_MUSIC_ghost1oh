...
@Client.on_message(command(["skip", f"skip@{BOT_USERNAME}", "vskip", "تخطي" ,"هات الي بعده"]) & other_filters)
@authorized_users_only
async def skip(client, m: Message):
    ...
    if len(m.command) < 2:
        ...
        if op == 0:
            await m.reply("❌ مفيش حاجه شغاله بقلبي")
        elif op == 1:
            await m.reply("✅ قوائم الانتظار ** فارغه.**\n\n**• خروج الحساب المساعد من المكالمه**")
        elif op == 2:
            await m.reply("🗑️ **مسح قوائم الانتظار**\n\n**• مغادره المستخدم من الدردشه الصوتيه**")
        else:
            await m.reply_photo(
                photo=f"{IMG_3}",
        ...
        OP = "🗑 **تمت ازاله الاغنيه من قائمه الانتظار:**"
        if chat_id in QUEUE:
...
@Client.on_message(
    command(["stop", f"stop@{BOT_USERNAME}", "end", f"end@{BOT_USERNAME}", "vstop", "عموري بس" ,"وقف"])
    & other_filters
)
@authorized_users_only
async def stop(client, m: Message):
    ...
    if chat_id in QUEUE:
        try:
            ...
            clear_queue(chat_id)
            await m.reply("✅ تم ايقاف التشغيل ي قلبي .")
        except Exception as e:
            await m.reply(f"🚫 **خطاء:**\n\n`{e}`")
    else:
        await m.reply("❌ **قائمه الاغاني مفهاش حاجه**")


@Client.on_message(
    command(["pause", f"pause@{BOT_USERNAME}", "vpause", "اقفل شاشه"]) & other_filters
)
@authorized_users_only
...
@Client.on_message(
    command(["mute", f"mute@{BOT_USERNAME}", "vmute", "شد ميوت"]) & other_filters
)
@authorized_users_only
